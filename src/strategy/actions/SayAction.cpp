    ...
    , const WorldPacket chatTemplate, const WorldPacket emoteTemplate, const WorldPacket systemTemplate, const std::string startPattern, const std::string endPattern, const std::string deletePattern, const std::string splitPattern, bool debug)
{
    ...
    std::string response = LLMInterface::Generate(json, sPlayerbotAIConfig.llmGenerationTimeout, sPlayerbotAIConfig.llmMaxSimultaniousGenerations, debugLines);

    std::vector<std::string> lines = LLMInterface::ParseResponse(response, startPattern, endPattern, deletePattern, splitPattern, debugLines);
...
void ChatReplyAction::ChatReplyDo(Player* bot, uint32& type, uint32& guid1, uint32& guid2, std::string& msg, std::string& chanName, std::string& name)
{
    ...
    if (bot->GetPlayerbotAI()
        ...
        || sPlayerbotAIConfig.llmEnabled == 3)
        && chatChannelSource != ChatChannelSource::SRC_UNDEFINED
        && sPlayerbotAIConfig.llmBlockedReplyChannels.find(chatChannelSource) == sPlayerbotAIConfig.llmBlockedReplyChannels.end()
    )
    {
        Player* player = sObjectAccessor.FindPlayer(ObjectGuid(HighGuid::Player, guid1));
        ...
        if (player)
        {
            ...
            if (player != bot && (player->isRealPlayer() || (sPlayerbotAIConfig.llmBotToBotChatChance && urand(0, 99) < sPlayerbotAIConfig.llmBotToBotChatChance)))
            {
                ...
                LLMInterface::LimitContext(llmContext, currentLength);
                ...
                for (auto& prompt : jsonFill)
                {
                    prompt.second = LLMInterface::SanitizeForJson(prompt.second);
                }
            ...
            else if (player != bot || sPlayerbotAIConfig.llmBotToBotChatChance)
            {
                ...
                LLMInterface::LimitContext(llmContext, llmContext.size());
            }
