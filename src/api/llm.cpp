...
inline void SetNonBlockingSocket(int sock) {
    ...
    if (ioctlsocket(sock, FIONBIO, &mode) != 0) {
        LOG_ERROR("playerbots", "Failed to set non-blocking mode on socket.");
    }
#else
...
#ifdef _WIN32
                LOG_ERROR("playerbots", "recv error: %s", WSAGetLastError());
#else
                LOG_ERROR("playerbots", "recv error: %s", strerror(errno));
#endif
                break;
            }
            }
...
std::string LLMInterface::Generate(const std::string& prompt, int timeOutSeconds, int maxGenerations, std::vector<std::string> & debugLines) {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        ...
        LOG_ERROR("playerbots", "WSAStartup failed");
        return "error";
    }
#endif
    ...
    if (getaddrinfo(parsedUrl.hostname.c_str(), std::to_string(parsedUrl.port).c_str(), &hints, &res) != 0) {
        ...
        LOG_ERROR("playerbots", "Failed to resolve hostname");
#ifdef _WIN32
        WSACleanup();
#endif
        return "error";
    ...
    if (sock == INVALID_SOCKET) {
        ...
        LOG_ERROR("playerbots", "Socket creation failed");
        WSACleanup();
        return "error";
    }
#else
    ...
    if (connect(sock, res->ai_addr, res->ai_addrlen) < 0) {
        ...
        LOG_ERROR("playerbots", "Connection to server failed");
#ifdef _WIN32
        closesocket(sock);
        WSACleanup();
#else
    ...
    if (parsedUrl.https)
    {
        ...
        if (!ctx) {
            ...
            LOG_ERROR("playerbots", "Failed to create SSL context");
#ifdef _WIN32
            closesocket(sock);
            WSACleanup();
#else
        ...
        if (SSL_connect(ssl) <= 0) {
            ...
            LOG_ERROR("playerbots", "SSL connection failed");
            SSL_free(ssl);
            SSL_CTX_free(ctx);
#ifdef _WIN32
            closesocket(sock);
    ...
    if (write) {
        ...
        LOG_ERROR("playerbots", "Failed to send request");
    ...
    if (bytesRead == SOCKET_ERROR) {
        ...
        LOG_ERROR("playerbots", "Error reading response");
    }
    closesocket(sock);
    WSACleanup();
#else
    if (bytesRead < 0) {
        if (debug)
            debugLines.push_back("Error reading response");
        LOG_ERROR("playerbots", "LOG_ERRORError reading response");
    }
    close(sock);
#endif
...
std::vector<std::string> LLMInterface::ParseResponse(const std::string& response, const std::string& startPattern, const std::string& endPattern, const std::string& deletePattern, const std::string& splitPattern, std::vector<std::string>& debugLines)
{
    bool debug = !(debugLines.empty());
...
void LLMInterface::LimitContext(std::string& context, int currentLength)
{
